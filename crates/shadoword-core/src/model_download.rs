...
mod tests {
    ...
    #[test]
    fn catalog_ids_and_artifacts_are_unique() {
        for (index, model) in WHISPER_MODELS.iter().enumerate() {
            assert_eq!(model.sha256.len(), 64);
            assert!(model.url.starts_with("https://huggingface.co/"));
            for other in &WHISPER_MODELS[index + 1..] {
    ...
    #[test]
    fn downloads_and_atomically_verifies_a_local_fixture() {
        ...
        let spec = WhisperModelSpec {
            ...
            url,
            sha256: "98a4ec349558836b02c63b578e51990695f1712c11d91790f6f9a2113a9ef385",
            size_bytes: 9,
            recommended: false,
        };
        let root =
            std::env::temp_dir().join(format!("shadoword-download-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        let mut last_progress = None;

        let status = download_whisper_model_with_progress(&spec, &root, |progress| {
        ...
        assert_eq!(
            fs::read(&status.path).expect("read downloaded fixture"),
            b"shadoword"
        );
        assert!(fs::read_dir(&root)
            .expect("read fixture directory")
            .all(|entry| entry.expect("fixture entry").path().extension()
                != Some(std::ffi::OsStr::new("part"))));
        assert_eq!(last_progress.map(|progress| progress.downloaded), Some(9));

        let skipped = download_whisper_model(&spec, &root).expect("verify existing fixture");
    ...
    #[test]
    fn verifies_known_bytes() {
        let root = std::env::temp_dir().join(format!("shadoword-sha-test-{}", std::process::id()));
        fs::create_dir_all(&root).expect("create temp directory");
        let path = root.join("fixture.bin");
        fs::write(&path, b"shadoword").expect("write fixture");

        verify_model_file(
            &path,
            "98a4ec349558836b02c63b578e51990695f1712c11d91790f6f9a2113a9ef385",
        )
        .expect("fixture checksum should match");
        assert!(verify_model_file(&path, &"0".repeat(64)).is_err());
