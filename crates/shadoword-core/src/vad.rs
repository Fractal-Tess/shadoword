...
mod tests {
    ...
    fn splits_speech_on_pause() {
        ...
        samples.extend(vec![0.08; sample_rate as usize]);
        ...
        assert!(segments.iter().all(|segment| segment.speech_ms >= 900));
    }

    #[test]
    fn preserves_short_words_separated_by_pauses() {
        let sample_rate = 16_000;
        let mut segmenter = VadSegmenter::new(sample_rate, VadSegmenterConfig::default());

        let mut samples = Vec::new();
        for _ in 0..3 {
            samples.extend(vec![0.08; samples_for_ms(sample_rate, 300)]);
            samples.extend(vec![0.0; samples_for_ms(sample_rate, 900)]);
        }

        let segments = segmenter.push_samples(&samples);

        assert_eq!(segments.len(), 3);
        assert!(segments
            .iter()
            .all(|segment| segment.speech_ms >= 270 && segment.speech_ms <= 330));
    }

    #[test]
    fn continuous_speech_waits_for_finish_without_force_split() {
        let sample_rate = 16_000;
        let mut segmenter = VadSegmenter::new(
            sample_rate,
            VadSegmenterConfig {
        ...
        let samples = vec![0.08; sample_rate as usize * 3];
        let segments = segmenter.push_samples(&samples);
        assert!(segments.is_empty());

        let tail = segmenter
            .finish()
            .expect("finish should flush continuous speech");
        assert!(tail.speech_ms >= 2_900);
    }

    #[test]
    fn short_noise_burst_followed_by_silence_is_discarded() {
        let sample_rate = 16_000;
        let mut segmenter = VadSegmenter::new(
            sample_rate,
            VadSegmenterConfig {
        ...
        let mut samples = Vec::new();
        samples.extend(vec![0.08; sample_rate as usize / 10]);
        samples.extend(vec![0.0; sample_rate as usize]);
