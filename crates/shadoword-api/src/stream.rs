...
mod tests {
    ...
    use shadoword_core::{
        ExecutionModelFactory, ExecutionTarget, ExecutionUnitConfig, InferenceLimits,
        InferencePoolConfig, InferenceRuntime, TranscriptionConfig,
    };
    use shadoword_shared::{
        LoadProgress, Model, ModelConfig, ModelError, Transcription, TranscriptionOptions,
    };
    ...
    impl ExecutionModelFactory for FakeFactory {
        fn create(&self, _unit: &ExecutionUnitConfig) -> Result<Box<dyn Model>, ModelError> {
    ...
    impl Model for FakeModel {
        ...
        fn load(&mut self, _config: &ModelConfig) -> Result<(), ModelError> {
            Ok(())
        }
        ...
        fn transcribe(
            ...
            let marker = input.samples[0] as i32;
            ...
            if marker == 9 {
                return Err(ModelError {
                    message: "planned segment failure".to_string(),
                });
            }
            Ok(Transcription {
    ...
    fn stream_state(control: Arc<FakeControl>) -> AppState {
        let units = vec![
            ExecutionUnitConfig {
                id: "fake-0".to_string(),
                enabled: true,
        ...
        let config = TranscriptionConfig {
            ...
            inference_pool: Some(InferencePoolConfig {
                units,
                limits: InferenceLimits {
                    max_outstanding_per_flow: 4,
        ...
        let runtime = InferenceRuntime::new_with_factory(config, Arc::new(FakeFactory { control }))
            .expect("prepare stream fake runtime");
        let directory = std::env::temp_dir().join(format!(
            "shadoword-stream-tests-{}",
        ...
        AppState {
            ...
            config_path: directory.join("api.json"),
            download_dir: directory,
            listen_addr: "127.0.0.1:47813".parse().expect("listen address"),
            queue_capacity: 4,
            config_update_lock: Arc::new(tokio::sync::Mutex::new(())),
        }
    }

    fn v2_session(state: &AppState) -> StreamSession {
        StreamSession::new(
            StartMessage {
                sample_rate: 16_000,
                channels: 1,
                protocol_version: 2,
                audio_format: StreamAudioFormat::Opus,
            },
            state,
        )
        .expect("start v2 session")
    }

    #[test]
    fn parses_v1_v2_and_selectable_v3_pcm_formats() {
        assert_eq!(
            parse_client_text(r#"{"type":"Start"}"#),
            Ok(ClientText::Start(StartMessage {
                sample_rate: 48_000,
                channels: 1,
                protocol_version: 1,
                audio_format: StreamAudioFormat::Opus,
        ...
        assert_eq!(
            parse_client_text(
                r#"{"type":"Start","sample_rate":16000,"channels":1,"protocol_version":2}"#
            ),
            Ok(ClientText::Start(StartMessage {
                sample_rate: 16_000,
                channels: 1,
        ...
        assert_eq!(
            parse_client_text("CommitSegment"),
            Ok(ClientText::CommitSegment {
                segment_index: None
            })
        );
        assert_eq!(
            parse_client_text(r#"{"type":"CommitSegment","segment_index":3}"#),
            Ok(ClientText::CommitSegment {
        ...
        assert_eq!(
            parse_client_text(
                r#"{"type":"Start","sample_rate":16000,"channels":1,"protocol_version":3,"audio_format":"pcm_f32le"}"#
            ),
            Ok(ClientText::Start(StartMessage {
                sample_rate: 16_000,
                channels: 1,
                protocol_version: 3,
                audio_format: StreamAudioFormat::PcmF32le,
            }))
        );
        assert_eq!(
            parse_client_text(
                r#"{"type":"Start","sample_rate":44100,"channels":1,"protocol_version":3,"audio_format":"pcm_s16le"}"#
            ),
            Ok(ClientText::Start(StartMessage {
                sample_rate: 44_100,
                channels: 1,
                protocol_version: 3,
                audio_format: StreamAudioFormat::PcmS16le,
            }))
        );
        assert_eq!(parse_client_text("Finish"), Ok(ClientText::Finish));
    }

    #[test]
    fn rejects_invalid_start_parameters_and_protocols() {
        assert!(parse_client_text(r#"{"type":"Start","sample_rate":44100}"#).is_err());
        assert!(parse_client_text(r#"{"type":"Start","channels":3}"#).is_err());
        assert!(parse_client_text(r#"{"type":"Start","protocol_version":4}"#).is_err());
        assert!(parse_client_text(r#"{"type":"Start","protocol_version":3}"#).is_err());
        for audio_format in ["pcm_f32le", "pcm_s16le"] {
            assert!(parse_client_text(&format!(
                r#"{{"type":"Start","protocol_version":2,"audio_format":"{audio_format}"}}"#
            ))
            .is_err());
        }
        assert!(parse_client_text(
            r#"{"type":"Start","sample_rate":44100,"protocol_version":3,"audio_format":"pcm_f32le"}"#
        )
        .is_ok());
        assert!(parse_client_text(
            r#"{"type":"Start","sample_rate":384001,"protocol_version":3,"audio_format":"pcm_s16le"}"#
        )
        .is_err());
    }

    #[test]
    fn raw_pcm_packets_drive_server_vad_and_force_flush_short_speech() {
        let state = stream_state(Arc::new(FakeControl::default()));
        ...
        .expect("start v3 session");
        let packet = |samples: &[f32]| {
            samples
                .iter()
        ...
        let mut segments = Vec::new();
        for chunk in vec![0.08_f32; 16_000 * 300 / 1_000]
            .chunks(160)
            .chain(vec![0.0_f32; 16_000 * 900 / 1_000].chunks(160))
        {
            segments.extend(
                session
                    .accept_packet(&packet(chunk))
                    .expect("accept raw PCM packet"),
            );
        }
        assert_eq!(segments.len(), 1);

        for chunk in vec![0.08_f32; 16_000 * 100 / 1_000].chunks(160) {
            assert!(session
                .accept_packet(&packet(chunk))
                .expect("accept short raw PCM packet")
                .is_empty());
        }
        assert!(session.force_finish_vad().is_some());
    }

    #[test]
    fn signed_16_bit_pcm_packets_drive_the_same_server_vad() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = StreamSession::new(
            StartMessage {
                sample_rate: 16_000,
        ...
        .expect("start signed 16-bit v3 session");
        let packet = |samples: &[i16]| {
            samples
                .iter()
                .flat_map(|sample| sample.to_le_bytes())
                .collect::<Vec<_>>()
        ...
        let mut segments = Vec::new();
        for chunk in vec![2_621_i16; 16_000 * 300 / 1_000]
            .chunks(160)
            .chain(vec![0_i16; 16_000 * 900 / 1_000].chunks(160))
        {
            segments.extend(
                session
                    .accept_packet(&packet(chunk))
                    .expect("accept signed 16-bit PCM packet"),
            );
        }

        assert_eq!(segments.len(), 1);
        assert_eq!(segments[0].audio.sample_rate, 16_000);
    }

    #[test]
    fn decodes_little_endian_signed_16_bit_pcm_to_normalized_f32() {
        let wire_samples = [i16::MIN, -1, 0, 1, i16::MAX];
        let packet = wire_samples
            .iter()
            .flat_map(|sample| sample.to_le_bytes())
            .collect::<Vec<_>>();

        assert_eq!(
            decode_pcm_s16le(&packet).expect("decode signed 16-bit PCM"),
            vec![
                -1.0,
                -1.0 / 32_768.0,
                0.0,
    ...
    #[test]
    fn rejects_misaligned_or_invalid_pcm_samples() {
        assert_eq!(
            decode_pcm_s16le(&[0])
                .expect_err("reject odd s16 packet")
                .message(),
            "pcm_s16le packet length must be divisible by 2"
        );
        assert!(decode_pcm_f32le(&[0, 1, 2]).is_err());
        assert!(decode_pcm_f32le(&f32::NAN.to_le_bytes()).is_err());
        assert!(decode_pcm_f32le(&f32::INFINITY.to_le_bytes()).is_err());
        assert!(decode_pcm_f32le(&1.000_001_f32.to_le_bytes()).is_err());
        assert!(decode_pcm_f32le(&(-1.000_001_f32).to_le_bytes()).is_err());
        assert_eq!(
            decode_pcm_f32le(
                &[
            ...
            .expect("decode finite normalized f32 PCM"),
            vec![-1.0, 0.0, 1.0]
        );
    }

    #[test]
    fn pcm_packet_duration_limit_uses_each_wire_sample_width() {
        assert_eq!(max_pcm_packet_bytes(16_000, PcmWireFormat::F32Le), 16_000);
        assert_eq!(max_pcm_packet_bytes(16_000, PcmWireFormat::S16Le), 8_000);
        assert_eq!(
            max_pcm_packet_bytes(384_000, PcmWireFormat::F32Le),
            MAX_OPUS_PACKET_BYTES
        );
        assert_eq!(
            max_pcm_packet_samples(384_000, PcmWireFormat::F32Le),
            16_384
        );
        assert_eq!(
            max_pcm_packet_samples(384_000, PcmWireFormat::S16Le),
            32_768
        );

        let state = stream_state(Arc::new(FakeControl::default()));
        for (audio_format, wire_format) in [
            (StreamAudioFormat::PcmF32le, PcmWireFormat::F32Le),
            (StreamAudioFormat::PcmS16le, PcmWireFormat::S16Le),
        ] {
            let mut session = StreamSession::new(
                StartMessage {
                    sample_rate: 16_000,
            ...
            .expect("start selectable v3 PCM session");
            let oversized = vec![
                0_u8;
                max_pcm_packet_bytes(16_000, wire_format)
                    + wire_format.bytes_per_sample()
            ];
            assert!(session.accept_packet(&oversized).is_err());
        }
    }

    #[test]
    fn bounds_raw_pcm_using_decoded_and_resampled_f32_memory() {
        assert_eq!(estimated_inference_audio_bytes(1, 8_000, 16_000), Some(12));
        assert_eq!(max_input_samples_for_inference(12, 8_000, 16_000), 1);
        assert_eq!(max_input_samples_for_inference(11, 8_000, 16_000), 0);
        assert_eq!(max_input_samples_for_inference(12, 32_000, 16_000), 2);

        let f32_segment =
            max_vad_segment_samples(64 * 1024 * 1024, 384_000, 384_000, PcmWireFormat::F32Le);
        let s16_segment =
            max_vad_segment_samples(64 * 1024 * 1024, 384_000, 384_000, PcmWireFormat::S16Le);
        assert_eq!(f32_segment - s16_segment, 16_384);
    }

    #[test]
    fn pending_vad_credit_queue_accounts_for_resampling_memory() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = StreamSession::new(
            StartMessage {
                sample_rate: 8_000,
        ...
        .expect("start s16 v3 session");
        session.max_pending_audio_bytes = 11;
        ...
        assert!(session.enqueue_vad_segment(segment.clone()).is_err());
        session.max_pending_audio_bytes = 12;
        session
            .enqueue_vad_segment(segment)
            .expect("queue one source sample and two resampled samples");
        assert_eq!(session.pending_audio_bytes, 12);
        session
            .pop_pending_vad_segment()
            .expect("pop queued segment");
        assert_eq!(session.pending_audio_bytes, 0);
    }

    #[test]
    fn downmixes_stereo_packets_to_mono_samples() {
        let mono = downmix_to_mono(&[0.5, -0.5, 1.0, 0.0], 2);

        assert_eq!(mono, vec![0.0, 0.5]);
    }

    #[tokio::test]
    async fn server_vad_segment_enters_the_ordered_inference_flow() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = StreamSession::new(
            StartMessage {
                sample_rate: 16_000,
        ...
        let mut segments = Vec::new();
        for chunk in vec![1.0_f32; 16_000 * 300 / 1_000]
            .chunks(160)
            .chain(vec![0.0_f32; 16_000 * 900 / 1_000].chunks(160))
        {
            segments.extend(
        ...
        let segment = segments.pop().expect("server VAD segment");
        let metadata = SegmentMetadata::from(&segment);
        session
            .commit_vad_segment(&state, &tx, segment)
            .await
            .expect("submit server VAD segment");
        let acceptance = session.acceptance(Some(metadata));
        assert_eq!(acceptance.segment_index, 0);
        assert_eq!(acceptance.metadata.unwrap().sample_rate, 16_000);

        let completion = rx.recv().await.expect("raw segment completion");
        session.outstanding.remove(&completion.sequence);
        let ready = session
            .complete(
                completion.sequence,
                completion.result.expect("raw inference result").response,
            )
            .expect("order raw completion");
        assert_eq!(ready[0].1.text, "segment-1");
    }

    #[tokio::test]
    async fn finish_drains_vad_segments_buffered_behind_full_credit() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = v2_session(&state);
        session.protocol_version = PROTOCOL_V3;
        session.credit = 1;
        session.max_pending_segments = 2;
        let (tx, mut rx) = mpsc::channel(2);
        session
            .commit_vad_segment(
                &state,
            ...
            .await
            .expect("submit first segment");
        session
            .enqueue_vad_segment(VadSegment {
                audio: AudioInput {
                    samples: vec![2.0],
                    sample_rate: 16_000,
                },
        ...
        session
            ...
            .expect("buffer force-flushed tail");
        session.finishing = true;
        assert_eq!(session.pending_segments.len(), 2);

        let first = rx.recv().await.expect("first completion");
        session.outstanding.remove(&first.sequence);
        session
            .complete(first.sequence, first.result.expect("first result").response)
            .expect("complete first segment");
        let second = session
            .pop_pending_vad_segment()
            .expect("pending second segment");
        session
            .commit_vad_segment(&state, &tx, second)
            .await
            .expect("submit second segment after credit release while finishing");
        assert_eq!(session.next_sequence, 2);
        assert_eq!(session.pending_segments.len(), 1);
    }

    #[tokio::test]
    async fn concurrent_c_before_b_completion_is_committed_in_order() {
        let control = Arc::new(FakeControl::default());
        let state = stream_state(Arc::clone(&control));
        let mut session = v2_session(&state);
        let (tx, mut rx) = mpsc::channel(4);

        session.samples = vec![1.0];
        session
            .commit_segment(&state, &tx, Some(0), false)
            .await
            .expect("commit B");
        session.samples = vec![2.0];
        session
            .commit_segment(&state, &tx, Some(1), false)
            .await
            .expect("commit C");

        let c = tokio::time::timeout(Duration::from_secs(2), rx.recv())
            .await
            .expect("C completion timeout")
            .expect("C completion channel");
        assert_eq!(c.sequence, 1);
        assert_eq!(control.max_active.load(Ordering::Acquire), 2);
        session.outstanding.remove(&c.sequence);
        assert!(session
            .complete(c.sequence, c.result.expect("C result").response)
            .expect("buffer C")
            .is_empty());

        let b = rx.recv().await.expect("B completion");
        assert_eq!(b.sequence, 0);
        session.outstanding.remove(&b.sequence);
        let ready = session
            .complete(b.sequence, b.result.expect("B result").response)
            .expect("commit B and C");
        assert_eq!(
            ready
                .iter()
                .map(|(index, response)| (*index, response.text.as_str()))
                .collect::<Vec<_>>(),
            vec![(0, "segment-1"), (1, "segment-2")]
        );
    }

    #[tokio::test]
    async fn segment_failure_cancels_later_work_and_suppresses_late_results() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = v2_session(&state);
        let (tx, mut rx) = mpsc::channel(4);
        session.samples = vec![9.0];
        session
            .commit_segment(&state, &tx, Some(0), false)
            .await
            .expect("commit failing segment");
        session.samples = vec![7.0];
        session
            .commit_segment(&state, &tx, Some(1), false)
            .await
            .expect("commit later segment");

        let failed = rx.recv().await.expect("failure completion");
        assert_eq!(failed.sequence, 0);
        assert!(failed.result.is_err());
        session.outstanding.remove(&failed.sequence);
        session
            .ordered
            .fail(0, "planned segment failure")
            .expect("terminate stream");
        session.cancel_outstanding();

        let late = rx.recv().await.expect("suppressed late completion");
        assert_eq!(late.sequence, 1);
        assert!(matches!(late.result, Err(InferenceError::Cancelled)));
        assert!(session.ordered.is_terminal());
        assert!(session.segments.is_empty());
    }

    #[test]
    fn stale_completion_failure_does_not_terminate_an_emitted_api_stream() {
        let state = stream_state(Arc::new(FakeControl::default()));
        let mut session = v2_session(&state);
        session
            .ordered
            ...
            .expect("emit first segment");

        assert!(!terminate_on_completion_failure(
            &mut session,
            0,
            &InferenceError::WorkerFailed("late failure".to_string()),
        ));
        assert!(!session.ordered.is_terminal());
        assert_eq!(session.ordered.next_sequence(), 1);
    }

    #[tokio::test]
    async fn disconnect_cancellation_keeps_running_unit_accounted_until_exit() {
        let control = Arc::new(FakeControl::default());
        let state = stream_state(Arc::clone(&control));
        let mut session = v2_session(&state);
        let (tx, mut rx) = mpsc::channel(2);
        ...
        session
            .commit_segment(&state, &tx, Some(0), false)
            .await
            .expect("commit slow segment");

        let deadline = Instant::now() + Duration::from_secs(1);
        while control.active.load(Ordering::Acquire) == 0 && Instant::now() < deadline {
            tokio::task::yield_now().await;
        }
        assert_eq!(state.runtime.status().running_jobs, 1);
        drop(session);
        assert_eq!(state.runtime.status().running_jobs, 1);

        let late = rx.recv().await.expect("cancelled completion");
        assert!(matches!(late.result, Err(InferenceError::Cancelled)));
        assert_eq!(state.runtime.status().running_jobs, 0);
    }
}
