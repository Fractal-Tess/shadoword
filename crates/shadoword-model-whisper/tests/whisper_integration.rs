...
fn write_markdown_report(
    ...
    markdown.push_str(&format!(
        "- Total audio seconds: `{:.2}`\n",
        metrics.total_audio_secs
    ));
    markdown.push_str(&format!(
        "- Total inference seconds: `{:.2}`\n",
        metrics.total_infer_secs
    ));
    markdown.push_str(&format!(
        "- Aggregate realtime factor: `{:.2}x`\n",
        metrics.realtime_x
    ));
    markdown.push_str(&format!(
        "- Average WER: `{:.2}%`\n\n",
        metrics.avg_wer * 100.0
    ));
...
fn write_backend_summary(bench_results_dir: &Path, backend: &str) {
    ...
    let summary_path =
        bench_results_dir.join(format!("whisper-model-size-comparison-{}.md", backend));
    let mut markdown = String::new();
    markdown.push_str(&format!(
        "# Whisper Model Size Comparison ({}, GPU {})\n\n",
...
fn word_error_rate(hypothesis: &str, reference: &str) -> f64 {
    ...
    for i in 1..=hyp_words.len() {
        ...
        for j in 1..=ref_words.len() {
            let cost = if hyp_words[i - 1] == ref_words[j - 1] {
                0
            } else {
                1
            };
...
#[test]
#[ignore = "requires an external Whisper model and writes benchmark output"]
