...
- **Astro** for static generation. No client-side JavaScript ships: there are
  zero `<script>` tags in the built page, and every interactive thing on it is a
  real form control.
...
- [ ] **The real domain.** `site` in `astro.config.mjs` is `https://shadoword.dev`,
      which is an assumption, and `og:image` / `og:url` are absolute against it.
      Set it before the first share.
...
`og:image` is a 1200×532 screenshot of the shipped hero, and it is regenerated by
shooting the built page rather than composed by hand, so it cannot drift from the
site it advertises. Its bottom edge is the scarlet band hairline under the hero.
